...
*/	
define('ZJPAS_ZIPJP_SERVICE_API', 'https://zipcode.milkyfieldcompany.com/api/v1/');
define('ZJPAS_ZIPJP_FRONTEND_SCRIPT', 'https://resources.milkyfieldcompany.com/zip-jp/js/mfczip_finder_wpplugin_v1.js');
...
class ZJPAS_Admin {
	...
	function __construct() {
		...
		load_plugin_textdomain("zip-jp-postalcode-address-search", false, basename(dirname(__FILE__)).'/languages');
	...
	public static function add_shortcodes() {
		if (function_exists('wpcf7_add_form_tag'))
			wpcf7_add_form_tag('mfczipbtn', array(__CLASS__, 'shortcode_handler'), true);
		else if (function_exists('wpcf7_add_shortcode')) {
			wpcf7_add_shortcode('mfczipbtn', array(__CLASS__, 'shortcode_handler'), true);
		} else {
	...
	public static function shortcode_handler($tag) {
		...
		$tagtext = "<button type='button' ".mb_ereg_replace('class:', "class='", $tag->name)."' >".$label."</button>" ;
		return $tagtext;
	}
	...
	public static function tag_generator() {
		...
		wpcf7_add_tag_generator('zipjpaddr', __('Find from postcode', 'zip-jp-postalcode-address-search'), 'wpcf7-tg-pane-btnaddress', array(__CLASS__, 'tg_pane_btnaddress'));

		wpcf7_add_tag_generator('zipjppost', __('Find from address', 'zip-jp-postalcode-address-search'), 'wpcf7-tg-pane-btnpostcode', array(__CLASS__, 'tg_pane_btnpostcode'));

		wpcf7_add_tag_generator('zipjptext', __('ZIPJP text', 'zip-jp-postalcode-address-search'), 'wpcf7-tg-pane-text', array(__CLASS__, 'tg_pane_text'));
	...
	function mfczipjp_dbcheck() {
		...
		if( $installed_ver != $zipjpcash_db_version ) {
			$sql = "CREATE TABLE " . $this->table_name_cash . " (
	...
	function fnAdminPrintStyleScripts() {
		...
		wp_enqueue_script('jquery-ui-tabs', array( 'jquery', 'jquery-ui-tabs' ));

		wp_enqueue_script( 'wp-color-picker' );

		wp_enqueue_script( 'my-admin-script', plugins_url("js/admin.js", __FILE__), array( 'wp-color-picker' ), false, true );
	}
	...
	function add_menu() {
		...
		add_menu_page(__('Postal Code Address Search', 'zip-jp-postalcode-address-search'),__('Postal Code Address Search', 'zip-jp-postalcode-address-search'), 'level_8', 'mfc_zipjp_general', array($this,'show_text_option_page'),$icon, 31);
	}
	...
	function show_text_option_page() {
		if ( isset($_POST['zjpas_mfczipjpservice_options'])) {
			...
			if (array_key_exists('zjpas_mfczipjpapi_apikey', $_POST)) {
				$zjpas_mfczipjpservice_options['apikey'] = sanitize_text_field($_POST['zjpas_mfczipjpapi_apikey']);
			}
			if (array_key_exists('zjpas_mfczipjpapi_zipcodewithhyphen', $_POST)) {
				$zjpas_mfczipjpservice_options['zipcodewithhyphen'] = sanitize_text_field($_POST['zjpas_mfczipjpapi_zipcodewithhyphen']);
			}
			  ...
			  <div id="tabs-1">
				<p>
					<form action="" method="post">
						...
						$apiaddress =  isset($opt['apiuri']) ? $opt['apiuri']: ZJPAS_ZIPJP_SERVICE_API;
						$mfczip_zipcodewithhyphen = isset($opt['zipcodewithhyphen']) ? $opt['zipcodewithhyphen']: "true";
						...
						<table class="form-table">
							<tr valign="top" class="zjpas_mfczip_individualscriptmode_disable">
								...
								<td><input name="zjpas_mfczipjpapi_apikey" type="text" id="zjpas_mfczipjpapi_apikey" value="<?php  echo esc_attr($apikey) ?>" class="regular-text" style="text-align: left; width: 100%;"/></td>
							</tr>
							<tr valign="top">
								<th scope="row"><label for="zjpas_mfczipjpapi_zipcodewithhyphen">郵便番号のハイフン区切り</label></th>
								<td><label style="margin-right: 1em;"><input name="zjpas_mfczipjpapi_zipcodewithhyphen" type="radio" id="zjpas_mfczipjpapi_zipcodewithhyphenon" value="true" <?php echo esc_attr($mfczip_zipcodewithhyphen=="true" ? 'checked="checked"' : "" ); ?> class="" style="text-align: left; "/> 区切る(000-0000の形式)</label><label><input name="zjpas_mfczipjpapi_zipcodewithhyphen" type="radio" id="zjpas_mfczipjpapi_zipcodewithhyphenonoff" value="false" <?php echo esc_attr($mfczip_zipcodewithhyphen=="true" ? "" : 'checked="checked"' ); ?> class="" style="text-align: left; "/>区切らない(0000000の形式)</label></td>
							</tr>
							<tr valign="top">
								<th scope="row"><label for="zjpas_mfczipjpapi_warnBgColor">入力項目のワーニング色</label></th>
								<td><input name="zjpas_mfczipjpapi_warnBgColor" type="text" id="zjpas_mfczipjpapi_warnBgColor" value="<?php  echo esc_attr($mfczip_warnBgColor) ?>" class="my-color-field" style="text-align: left; width: 100%;" /></td>
		
							</tr>
							<tr valign="top" class="zjpas_mfczip_individualscriptmode_disable">
								<th scope="row"><label for="zjpas_mfczipjpapi_apiuri">API Address</label></th>
								<td><input name="zjpas_mfczipjpapi_apiuri" type="text" id="zjpas_mfczipjpapi_apiuri" value="<?php  echo esc_url($apiaddress) ?>" class="regular-text" style="text-align: left; width: 100%;"/></td>
							</tr>
							<tr valign="top">
								<th scope="row"><label for="zjpas_mfczipjpapi_pluginscript">スクリプト動作モード<br>(高度な設定)</label></th>
								<td><label style="margin-right: 1em;"><input name="zjpas_mfczipjpapi_pluginscript" type="radio" id="zjpas_mfczipjpapi_pluginscripton" value="true" <?php echo esc_attr($mfczip_pluginscriptmode=="true" ? 'checked="checked"' : "" ); ?> class="" style="text-align: left; "/> Wordpress標準のajaxを利用する</label><label><input name="zjpas_mfczipjpapi_pluginscript" type="radio" id="zjpas_mfczipjpapi_pluginscriptoff" value="false" <?php echo esc_attr($mfczip_pluginscriptmode=="true" ? "" : 'checked="checked"' ); ?> class="" style="text-align: left; "/>マイページからダウンロードしたスクリプトを利用する</label><br><span class="caution">Wordpress標準のajax ( /wp-admin/admin-ajax.php ) が利用できない場合、マイページからダウンロードした組み込み用スクリプトをサーバに配置してから「マイページからダウンロードしたスクリプトを利用する」に設定してください。</span></td>
							</tr>
						</table>
						<style>
							.caution {
			  ...
			  <div id="tabs-2">
				<p>
					...
					<p>
						...
						<img src="<?php echo esc_url(plugins_url("images/buttons.png", __FILE__)); ?>" style="max-width: 515px;"><br>
					...
					<p>
						<img src="<?php echo esc_url(plugins_url("images/addrfind.png", __FILE__)); ?>" style="max-width: 178px;"><br>
						郵便番号から住所検索ボタンのタグを生成します。<br>
						ボタンをクリックすると下図ダイアログを表示します。<br>
						<img src="<?php echo esc_url(plugins_url("images/addressfinddialog.png", __FILE__)); ?>" style="max-width: 630px;"><br>
					...
					<p>
						<img src="<?php echo esc_url(plugins_url("images/postcodefind.png", __FILE__)); ?>" style="max-width: 211px;"><br>
						住所(地名)から郵便番号検索ボタンのタグを生成します。<br>
						ボタンをクリックすると下図ダイアログを表示します。<br>
						<img src="<?php echo esc_url(plugins_url("images/postcodefinddialog.png", __FILE__)); ?>" style="max-width: 630px;"><br>
						ラベルがボタンのタイトルになるので、ご希望に応じて変更してください。<br>
						ID属性やクラス属性は任意に入力してください。<br>
						タグを挿入 ボタンをクリックすると、下記のようなコードが挿入されます。<br>
					...
					<p>
						<img src="<?php echo esc_url(plugins_url("images/zipjptext.png", __FILE__)); ?>" style="max-width: 126px;"><br>
						郵便番号検索機能と連携した単一行のプレーン入力フィールドのフォームタグを生成します。<br>
						ボタンをクリックすると下図ダイアログを表示します。<br>
						<img src="<?php echo esc_url(plugins_url("images/zipjptextdialog.png", __FILE__)); ?>" style="max-width: 630px;"><br>
						...
						<br>
						郵便番号に対応します。
						<div class="sourcecode"><span class="">[text* company-postalcode class:mfczip_postcode]</span></div>
						<br>
						都道府県に対応します。
						<div class="sourcecode"><span class="">[text* company-pref class:mfczip_prefname]</span></div>
						<br>
						町名に対応します。
						<div class="sourcecode"><span class="">[text* company-choumei class:mfczip_address]</span></div>
						<br>
						都道府県(カナ)を出力します。
						<div class="sourcecode"><span class="">[text* company-prefkana class:mfczip_prefkana]</span></div>
						<br>
						町名(カナ)を出力します。
						<div class="sourcecode"><span class="">[text* company-choumeikana class:mfczip_addresskana]</span></div>
						<br>
						事業所名を出力します。
						<div class="sourcecode"><span class="">[text* company-name class:mfczip_companyname]</span></div>
						<br>
						都道府県 と 町名 は組み合わせて指定できます。<br>
						また、都道府県(カナ) と 町名(カナ) も同様に組み合わせて指定できます。
						<div class="sourcecode"><span class="">[text* company-address class:mfczip_prefname class:mfczip_address]</span>
<span class="">[text* company-address-kana class:mfczip_prefkana class:mfczip_addresskana]</span></div>
					</p>
...
?>
		...
		<script>
			mfczip_zipcodewithhyphen = <?php  echo esc_js($mfczip_zipcodewithhyphen) ?>;
			mfczip_warnBgColor = '<?php  echo esc_js($mfczip_warnBgColor) ?>';
			mfczip_api_serviceurl = '<?php  echo esc_url($mfczip_api_serviceurl) ?>';
		</script>
...
<?php
	...
	function fnApiCall() {
		...
		$wpdb->query(
			$wpdb->prepare( 
				"
				DELETE FROM " . $this->table_name_cash . "
				WHERE execdatetime <= " . $older
			)
		);
		
		// キャッシュの検索
		$method = sanitize_text_field($_POST['method']);
		$zipcode = (array_key_exists('zipcode', $_POST) ? sanitize_text_field($_POST['zipcode']) : null );
		$address = (array_key_exists('address', $_POST) ? sanitize_text_field($_POST['address']) : null );;
		$wpdb->get_row( $query, $output_type, $row_offset ); 
		$resulttext = $wpdb->get_col(
			$wpdb->prepare( 
				"
				SELECT	  resulttext
				FROM	   ".$this->table_name_cash."
				WHERE	   method = %s ". ( $zipcode != null ? "AND zipcode = '$zipcode'" : "AND zipcode IS NULL " ).( $address != null ? "AND address = '$address' " : "AND address IS NULL " )."
		...
		if ( $resulttext ) 
		{
			...
			$opt = get_option('zjpas_mfczipjpservice_options');
			$apikey = isset($opt['apikey']) ? $opt['apikey']: "";
			$apiaddress =  isset($opt['apiaddress']) ? $opt['apiaddress']: ZJPAS_ZIPJP_SERVICE_API;
			...
			$ret = wp_remote_post( $API_URL, array(
				'method'=>'POST',
				'body'=>$postdata
			) );
			if ( is_wp_error( $ret ) ) {
			    $error_message = $response->get_error_message();
				$ret['message']= 'サービスは利用できません。(リクエストに失敗しました。['.$error_message.'])';
				wp_send_json($ret);
			} else {
				try {
